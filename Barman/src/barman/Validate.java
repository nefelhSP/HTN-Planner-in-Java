 ...
 * current state. If the preconditions are not satisfied the plan is invalid.
 * If the preconditions are satisfied the effects are applied to the current
 * state. It also includes a toggle switch between detailed output and minimal
 * output
 */
public class Validate {
    ...
    public Validate(boolean detailedOutput) {
    ...
    public static class ValidationResult {
         ...
         */
        public ValidationResult(boolean isValid, List<String> log, String failureReason, int failureStep, WorldState finalState) {
    ...
    public ValidationResult validate(WorldState initialState, List<Action> plan) {
        ...
        for (int i = 0; i < plan.size(); i++) {
            ...
            if (!preconditionChecker.checkPreconditions(action, currentState)) {
            ...
            effectsApplier.applyEffects(currentState, action.getEffects());
            if (detailedOutput) {
    ...
    public static void printValidationResult(ValidationResult result) {
        ...
        System.out.println("\n Is the given Plan, valid?: ");
        if (!result.isValid) {
            System.out.println("                             NO, the Plan is INVALID!");
            System.out.println("Failure at step: " + result.failureStep);
        } else {
            System.out.println("                             YES, the Plan is VALID!");
        }
    }
}
